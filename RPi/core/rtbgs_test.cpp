...
// g++ -o rtbgs_test rtbgs_test.cpp `pkg-config --cflags --libs opencv` -lpigpio -lrt -lpthread
...
int main() {
    ...
    while (true) {
        ...
        result_frame_vec = readData(handle_0, current_img_address, 3 * 16384);
        cv::Mat temp(128, 128, CV_8UC3, result_frame_vec.data());
        result_frame = temp;
